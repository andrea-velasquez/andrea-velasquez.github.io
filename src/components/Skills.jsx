...
class Skills extends Component {
  ...
  encode = (n) => {
    return n.toLowerCase().replace(" ", "-").replace("/", "-").replace(".", "");
  };
      ...
      <section id="Skills">
        ...
        <div className="section__content">
          {mainSkills.map((skillCategory, catIdx) => (
            <div
              key={catIdx}
              className={`field field__group field__${skillCategory.category.toLowerCase()}`}
            >
              <h4 className="field__name">
                {constants[skillCategory.category]}
              </h4>
          ...
          <div className="field field__others">
            ...
            <p className="field__text">{others.join(" - ")}</p>
          </div>
        </div>
      </section>
    );
